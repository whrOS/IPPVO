% function [ NNLL,xposxpos,yposypos,pphh,ppll ] = edge_point( I )
% 
% [A,B] = size(I); 
% % 处理第二行（除去左上角的点）
% NL = zeros(1,A*B);
...
%% 边缘像素就用3*3的块来算，不扩展成5*5
function [ NNLL,xposxpos,yposypos,pphh,ppll,zhizhi ] = edge_point( I )
...
pphh=[ph_x_a ph_x_b ph_x_c ph_x_d];
ppll=[pl_x_a pl_x_b pl_x_c pl_x_d];
zhizhi = [zhixa zhixb zhixc zhixd];
end
